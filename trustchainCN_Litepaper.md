...
8. [治理与升级](#8-治理与升级)
   - [8.4.5 代币价值与平台采用飞轮](#845-代币价值与平台采用飞轮)
   - [8.4.6 创业者从代币增值中受益](#846-创业者从代币增值中受益)
9. [典型应用场景](#9-典型应用场景)
   - [9.5 电商平台去中心化支付宝 (B2B)](#95-电商平台去中心化支付宝-b2b)
   - [9.6 Agent 对 Agent 经济 (A2A 交易)](#96-agent-对-agent-经济-a2a-交易)
   - [9.7 面向创业者的信任即服务 (TaaS)](#97-面向创业者的信任即服务-taas)
10. [x402 迁移路径与独立部署](#10-x402-迁移路径与独立部署)
    - [10.0 TrustApp 作为独立协议](#100-trustapp-作为独立协议)
11. [路线图](#11-路线图)
12. [结语](#12-结语)

**附录**
- [A-0. 证据标准与仲裁字典（Evidence Schema）](#附录-a-0证据标准与仲裁字典evidence-schema)
...
2. 进入挑战期（challengeWindow），期间 Payer 可发起争议。
...
- **单次争议**：每笔 `settlementId` 仅允许一次 `openDispute()`，且必须发生在 `challengeWindow` 内。
- **仲裁终局**：一旦进入 `FINALIZED`（含 `submitDecision()` 或超时默认裁决），结算即为终局，不可再次挑战或重开争议。
- **上诉边界**：如需上诉，必须在仲裁模块内部处理（见 5.4 上诉机制），不重新打开挑战期。
- **快速路径终局**：`settleWithConfirm(...)` 跳过挑战期并立即终局，无法再发起争议。

### 5.3 状态转换规则与交叉 Deadline 处理

#### 5.3.1 精确状态转换表

| 当前状态 | 事件/条件 | 目标状态 | 执行动作 |
|----------|-----------|----------|----------|
| SETTLE_PENDING | `openDispute()` 且 `block.timestamp < settleTime + challengeWindow` | DISPUTE_OPEN | 锁定结算资金，记录 disputeId |
| SETTLE_PENDING | `block.timestamp >= settleTime + challengeWindow` 且无争议 | FINALIZED | 释放资金给 Provider |
| DISPUTE_OPEN | `payBond()` 由双方调用 | BONDING | 记录保证金 |
| BONDING | 双方保证金到位 且 `block.timestamp < bondDeadline` | EVIDENCE | 开启证据窗口 |
| BONDING | `block.timestamp >= bondDeadline` 且 Payer 未付 | FINALIZED | Provider 胜诉，退还 Provider 保证金 |
| BONDING | `block.timestamp >= bondDeadline` 且 Provider 未付 | FINALIZED | Payer 胜诉，退还 Payer 保证金 |
| BONDING | `block.timestamp >= bondDeadline` 且双方均未付 | FINALIZED | INVALID，资金进入国库 |
| EVIDENCE | `submitEvidence()` 且 `block.timestamp < evidenceDeadline` | EVIDENCE | 记录证据哈希 |
| EVIDENCE | `block.timestamp >= evidenceDeadline` | DECISION | 自动转入裁决期 |
| DECISION | `submitDecision()` 由授权仲裁者调用 | FINALIZED | 执行裁决，分配资金 |
| DECISION | `block.timestamp >= decisionDeadline` | EXPIRED | 应用 defaultOutcome |
| EXPIRED | `forceFinalize()` 且 `block.timestamp >= decisionDeadline + gracePeriod` | FINALIZED | 强制结算 |

#### 5.3.2 交叉 Deadline 处理规则

**问题场景**：如果 bondWindow 还没结束，但证据已提前提交，系统如何处理？
...
│  规则 1：窗口不可跳跃                                            │
│  ─────────────────────                                          │
│  • 即使证据提前提交，仍需等待 bondWindow 结束                    │
│  • 状态转换严格按序：BONDING → EVIDENCE → DECISION               │
│                                                                  │
│  规则 2：提前提交的证据有效                                       │
│  ─────────────────────────                                       │
│  • 在 BONDING 期间提交的证据被缓存                               │
│  • 进入 EVIDENCE 状态后自动生效                                  │
│  • evidenceDeadline 从 EVIDENCE 状态开始计算                     │
│                                                                  │
│  规则 3：单边超时即判负                                          │
│  ──────────────────────                                          │
│  • 任一方在 bondWindow 内未付保证金 → 该方败诉                   │
│  • 无需等待证据期或裁决期                                        │
│                                                                  │
│  规则 4：宽限期保护                                              │
│  ───────────────────                                             │
│  • 所有 deadline 后有 1 小时 gracePeriod                         │
│  • gracePeriod 内的提交仍有效，但扣除延迟罚金（保证金的 5%）      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
─────────────────────────────────────────────────────────────────────►
     │              │                │               │
     │              │                │               │
  DISPUTE_OPEN   bondDeadline    evidenceDeadline  decisionDeadline
     │              │                │               │
     ▼              ▼                ▼               ▼
┌─────────┐   ┌──────────┐    ┌──────────┐    ┌──────────┐
│ BONDING │──►│ EVIDENCE │───►│ DECISION │───►│FINALIZED │
└─────────┘   └──────────┘    └──────────┘    └──────────┘
     │              │                │
     │   证据提前    │                │
     │   提交缓存 ───┘                │
...
**核心原则**：挑战的成功/失败由**预先注册的仲裁模块**决定，而非单一实体。
...
│  • 单边未提交证据 → 未提交方败诉                                  │
│  • 双边均超时 → INVALID，资金进国库                              │
│  执行者：Settlement 合约自动执行                                  │
│                                                                  │
│  Level 2: 模块裁决（按 arbMode）                                  │
│  ──────────────────────────────                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                                                          │    │
│  │  arbMode = TEE                                           │    │
│  │  ────────────                                            │    │
│  │  裁决者：TEE 验证合约                                     │    │
│  │  输入：TEEEvidence（attestation, mrenclave, 签名）        │    │
│  │  判定：签名有效且 mrenclave 在白名单 → Provider 胜诉       │    │
│  │        否则 → Payer 胜诉                                  │    │
│  │                                                          │    │
│  │  arbMode = ZK                                            │    │
│  │  ────────────                                            │    │
│  │  裁决者：ZK Verifier 合约                                 │    │
│  │  输入：ZKEvidence（proof, public inputs）                 │    │
│  │  判定：proof 验证通过 → Provider 胜诉                     │    │
│  │        否则 → Payer 胜诉                                  │    │
│  │                                                          │    │
│  │  arbMode = Oracle                                        │    │
│  │  ────────────────                                        │    │
│  │  裁决者：Oracle Aggregator 合约                           │    │
│  │  输入：多个 Oracle 签名的数据                             │    │
│  │  判定：≥2/3 Oracle 确认交付 → Provider 胜诉               │    │
│  │        否则 → Payer 胜诉                                  │    │
│  │                                                          │    │
│  │  arbMode = Committee                                     │    │
│  │  ──────────────────                                      │    │
...
| 仲裁模式 | 选择机制 | 激励 | 惩罚 |
|----------|----------|------|------|
| **TEE** | 白名单验证合约 | 无（纯链上验证） | 无 |
| **ZK** | 预部署 Verifier | 无（纯链上验证） | 无 |
| **Oracle** | 质押排名 + 随机选择 | 仲裁费分成 | 错误裁决罚没质押 |
| **Committee** | 治理选举 + 质押门槛 | 仲裁费分成 | 错误裁决罚没质押 + 移除资格 |
...
在仅由 Payer 发起争议的模型下，挑战成功率主要取决于：
- Payer 是否主动监测与举证
- 证据标准是否可验证（如 TEE/ZK/Oracle）
- challengeWindow 与保证金参数是否覆盖业务风险

**建议**：高价值交易优先选择可验证模式（TEE/ZK/Oracle）并配置更长 challengeWindow 与更高保证金门槛。
...
**L2 - 争议可发起**
> 在 challengeWindow 内，Payer 可以发起一次争议；一旦 FINALIZED，不可再次挑战。
...
TrustApp 协议可引入原生代币 $TRUST，用于协议安全层的激励对齐与治理。

#### 8.4.1 设计原则

**双层分离**：服务结算与协议安全使用不同资产。

| 层级 | 资产 | 用途 |
|------|------|------|
| 结算层 | 稳定币 (USDC) | Payer 托管、Provider 担保、服务计价 |
| 安全层 | $TRUST | 仲裁者质押、协议治理、价值捕获 |

这种分离确保服务价格不受代币波动影响，同时为协议安全提供经济激励。

#### 8.4.2 代币用途

**1. 仲裁者质押**

仲裁者必须质押 $TRUST 才能参与裁决：

| 参数 | 描述 |
|------|------|
| 最低质押 | 参与仲裁的门槛 |
| 可裁决上限 | 质押量决定可处理的争议金额上限 |
| 罚没机制 | 错误/恶意裁决触发质押罚没 |
...
| 来源 | 描述 |
|------|------|
| Protocol Fee | 每笔结算收取费用 |
| Arbitration Fee | 争议仲裁费用 |

**价值捕获机制**：

| 机制 | 描述 |
|------|------|
| 回购销毁 | 协议收入的一部分用于回购并销毁 $TRUST |
| 罚没销毁 | 仲裁者被罚没的 $TRUST 部分销毁 |
| 质押需求 | 仲裁者需持续持有 $TRUST |

#### 8.4.4 不使用 $TRUST 的场景

以下场景明确使用稳定币，避免代币波动影响服务可靠性：

| 场景 | 资产 | 原因 |
|------|------|------|
| Payer 托管 | USDC | 服务价值需稳定 |
| Provider 担保 | USDC | 担保价值需可预测 |
| 服务计价与结算 | USDC | 成本需确定性 |
| 争议赔偿 | USDC | 赔偿金额需稳定 |
...
$TRUST 代币的设计使其能够捕获 TrustApp 生态系统增长带来的价值。与纯效用代币不同，$TRUST 受益于多个与平台使用量挂钩的需求驱动因素。

**需求驱动因素**：

| 驱动因素 | 机制 | 扩展因子 |
|----------|------|----------|
| **仲裁者质押** | 更多争议 → 需要更多仲裁者 → 更多 $TRUST 被质押 | 与争议量线性相关 |
| **治理参与** | 更多利益相关者 → 更多治理需求 | 网络效应 |
| **平台集成** | 电商平台为高级功能质押 | 随 B2B 采用指数增长 |
...
```
日协议收入 = V * 0.003
日回购金额 = V * 0.003 * 0.5 = V * 0.0015
年回购金额 = V * 0.0015 * 365 = V * 0.5475
...
所需仲裁者质押（1%争议率）：
  争议量 = V * 0.01 = 10万美元/日
  所需仲裁者质押 ≈ 3x 争议价值 = 30万美元质押
  以 10% APY 计，仲裁者从此质押年收益 3万美元

$TRUST 总需求 = 回购 + 质押 = 结构性增值
```

**B2B 乘数效应**：

当电商平台集成 TrustApp 时，乘数效应加速：

| 平台规模 | 日交易量 | 年回购金额 | 仲裁者质押 |
|----------|----------|------------|------------|
| 小型市场 | 10万美元 | 5.475万美元 | 3千美元 |
| 中型市场 | 100万美元 | 54.75万美元 | 3万美元 |
| 大型市场 | 1000万美元 | 547.5万美元 | 30万美元 |
| 企业平台 | 1亿美元 | 5475万美元 | 300万美元 |

**一个大型企业采用 = 100个小型市场对代币需求的影响。**

#### 8.4.6 创业者从生态系统增长中受益

创业者（Provider）使用 USDC 质押以保证服务交付，同时可通过持有 $TRUST 参与协议治理和价值捕获：

| 收益 | 描述 |
|------|------|
| **服务担保稳定** | Provider 使用 USDC 质押，担保价值不受代币波动影响 |
| **$TRUST 治理权** | 持有 $TRUST 可参与协议参数、费率、模块审核等治理 |
| **成为仲裁者** | 质押 $TRUST 可参与仲裁，赚取额外收入 |
| **声誉可携带** | 链上交易记录可跨平台验证，降低新平台冷启动成本 |
...
│  服务层（必须）                    治理层（可选）                  │
│  ─────────────                    ─────────────                  │
│  质押 USDC 作为服务担保            持有 $TRUST 参与治理            │
│  ↓                                ↓                              │
│  提供服务、赚取服务费              投票决定协议参数                │
│  ↓                                ↓                              │
│  建立链上声誉                      质押 $TRUST 成为仲裁者          │
│                                   ↓                              │
│                                   赚取仲裁费分成                  │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
这种设计确保：
- **服务层安全**：USDC 担保价值稳定，不受市场波动影响
- **治理层激励**：$TRUST 持有者利益与协议成功绑定
- **可选参与**：创业者可仅参与服务层，无需持有 $TRUST
...
TrustApp 使任何电商平台都能构建自己的**去中心化托管与仲裁系统**——本质上是无需中心化控制的"去中心化支付宝"。

**对平台的价值主张**：

| 传统方式 | TrustApp 方式 |
|----------|---------------|
| 自建托管系统 | 使用 TrustApp 合约作为基础设施 |
| 雇佣仲裁人员 | 接入去中心化仲裁网络 |
| 承担监管风险 | 中立协议，规则透明 |
| 仅限国内管辖 | 通过经济惩罚实现跨境执行 |
| 高运营成本 | 按使用付费的协议费用 |
...
| 收入来源 | 描述 |
|----------|------|
| **平台费用加成** | 在 TrustApp 协议费用上加收平台利润 |
| **高级仲裁** | 提供更快/专业的仲裁服务，收取更高费用 |
| **质押服务** | 帮助商户质押并赚取仲裁参与奖励 |
| **白标方案** | 向其他平台授权定制化 TrustApp 集成 |

**流程**：
1. 平台集成 TrustApp SDK
2. 买家购买商品；资金锁定在 TrustApp 托管
3. 卖家发货；物流 Oracle 确认送达
4. 确认后自动结算；或争议 → 仲裁
5. 平台从每笔成功交易中赚取费用

**适用仲裁模式**：Oracle（物流）+ Committee（质量争议）

**部署案例**：
- 东南亚电商平台集成 TrustApp
- 中国卖家可服务印尼买家（跨境交易）
- 无需本地支付牌照——TrustApp 处理托管
- 争议由去中心化仲裁者解决，而非平台员工
- 平台专注于获客，而非支付基础设施

### 9.6 Agent 对 Agent 经济 (A2A 交易)

随着 AI Agent 越来越多地执行自主交易，TrustApp 为 **Agent 对 Agent 商务**提供信任层。

**Agent 经济的挑战**：

| 挑战 | 无 TrustApp | 有 TrustApp |
|------|-------------|-------------|
| Agent A 向 Agent B 付费获取服务 | B 失败无追索权 | 托管 + 仲裁 |
| 计费准确性 | 信任 B 的计量 | 可验证收据 |
| 质量争议 | 无解决路径 | Committee/ZK 仲裁 |
| Agent 声誉 | 主观、可操纵 | 质押支撑的经济问责 |
...
|----------|----------|----------|----------|
| **Tier 0** | 无 | $10/笔 | 演示、测试 |
| **Tier 1** | 100 USDC | $1,000/笔 | 个人 Agent |
| **Tier 2** | 1,000 USDC | $10,000/笔 | 商业 Agent |
| **Tier 3** | 10,000 USDC | $100,000/笔 | 企业 Agent |

**Agent 运营者的收入模式**：
- Agent B（提供者）按交易收取服务费
- TrustApp 协议收取小额百分比（0.3%）
- Agent B 质押 $TRUST 或 USDC 以启用更高交易限额
- 成功的交易记录可随时间降低质押要求

**适用仲裁模式**：TEE（可验证计算）或 ZK（可证明执行）

### 9.7 面向创业者的信任即服务 (TaaS)

TrustApp 开创了一个新类别：**信任即服务**——让数百万创业者无需自建信任基础设施即可提供经过验证的可信服务。
...
| 创业者类型 | 传统障碍 | TrustApp 解决方案 |
|------------|----------|-------------------|
| **自由开发者** | 客户不信任未知承包商 | 质押支撑的交付保证 |
| **小型制造商** | 买家担心不发货 | 托管 + 物流 Oracle |
| **AI 服务提供商** | 用户质疑计算准确性 | TEE/ZK 验证执行 |
| **内容创作者** | 买家担心质量问题 | Committee 仲裁 |

**创业者如何使用 TrustApp**：

1. **即时建立信任**：质押担保 → 立即有资格进行验证交易
2. **无声誉冷启动**：经济质押替代声誉要求
3. **全球触达**：无需本地牌照的跨境交易
4. **争议保护**：仲裁也保护诚实的提供者
...
|----------|------|------|
| 协议费用 | 每笔结算 0.3% | 协议维护 |
| 质押要求 | 交易价值的 1.5-3 倍 | 经济安全 |
| 仲裁费用 | 0.5-2%（仅发生争议时） | 争议解决 |

**净收益**：即使有这些成本，创业者也获得了接触那些在没有信任保证时永远不会交易的客户的机会。
...
**适用仲裁模式**：根据服务类型而异（见模式选择指南）

---

## 10. x402 迁移路径与独立部署

### 10.0 TrustApp 作为独立协议

**TrustApp 不依赖 x402**。虽然 TrustApp 可以增强基于 x402 的支付流程，但它是一个完全独立的协议，本身就能提供价值：

| 部署模式 | 描述 | 适用场景 |
|----------|------|----------|
| **独立 TrustApp** | 不使用 x402 的完整协议 | 电商、B2B 服务、Agent 网络 |
| **TrustApp + x402** | x402 作为支付触发器，TrustApp 作为执行层 | 具有现有 x402 基础设施的 API 计费 |
| **TrustApp + 自定义集成** | 直接 SDK/合约集成 | 企业平台、自定义市场 |

**为什么独立运行有效**：
- TrustApp 的核心价值是**信任基础设施**，而非支付触发
- 任何支付方式（直接转账、托管、订阅）都可以使用 TrustApp 的验证和仲裁
- x402 只是一种可能的支付触发方式；TrustApp 同样适用于直接合约调用、SDK 集成或自定义协议

TrustApp 也为希望逐步增加信任保障的现有 x402 用户提供兼容层。
...
| **阶段 2** | Receipt 批量结算 + 争议工具 | 降低 Gas，改善争议体验 |
...
## 附录 A-0：证据标准与仲裁字典（Evidence Schema）

为使仲裁具有可操作性，必须定义每种服务类型的**证据标准**——即 UsageReceipt 应包含哪些字段，以及仲裁者如何判定"交付成功"。

### A-0.1 证据标准设计原则

| 原则 | 描述 |
|------|------|
| **确定性** | 相同输入 + 相同处理 = 相同输出（可重放验证） |
| **不可伪造** | 证据必须包含难以伪造的签名/哈希/时间戳 |
| **最小充分** | 仅包含裁决所需的最少信息，保护隐私 |
| **模块化** | 不同服务类型使用不同证据模板，通过 moduleId 区分 |

### A-0.2 服务类型与证据模板

#### 模板 1：AI 推理服务（AI Inference）

**场景**：用户请求 AI 模型生成内容（文本、图像、代码）

```solidity
struct AIInferenceReceipt {
    // === 基础字段（所有 Receipt 通用）===
    bytes32 requestId;
    bytes32 receiptId;
    address payer;
    ...
    // === AI 推理专用字段 ===
    bytes32 modelId;           // 模型标识（如 "gpt-4", "stable-diffusion-xl"）
    bytes32 inputHash;         // 输入内容哈希（prompt hash）
    bytes32 outputHash;        // 输出内容哈希
    uint32 inputTokens;        // 输入 token 数
    uint32 outputTokens;       // 输出 token 数（或图片尺寸）
    bytes32 seed;              // 随机种子（用于可重放验证）
    bytes32 configHash;        // 推理参数哈希（temperature, top_p 等）

    // === 可验证性字段 ===
    bytes32 executionEnvHash;  // 执行环境哈希（TEE mrenclave 或容器 ID）
    bytes signature;           // Provider 对上述字段的签名
}
```
...
| 仲裁模式 | 判定逻辑 |
|----------|----------|
| **TEE** | 验证 executionEnvHash 在白名单 + 签名有效 → Provider 胜诉 |
| **ZK** | 重放 (inputHash, seed, configHash) 生成 outputHash，匹配则 Provider 胜诉 |
| **Committee** | 人工审核输出质量是否符合 prompt 意图 |

**防伪造设计**：
- `seed` 必须由 Payer 提供或由 VRF 生成，防止 Provider 选择性出示"好结果"
- `inputHash` 由 Payer 签名确认，防止 Provider 事后修改 prompt
- `executionEnvHash` 需与 TEE 远程证明匹配

---

#### 模板 2：AI 图像生成服务（AI Image Generation）

**场景**：用户请求 AI 生成图像（Stable Diffusion, DALL-E 等）

```solidity
struct ImageGenerationReceipt {
    // === 基础字段 ===
    bytes32 requestId;
    bytes32 receiptId;
    address payer;
    ...
    // === 图像生成专用字段 ===
    bytes32 modelId;           // 模型 ID（如 "sd-xl-1.0"）
    bytes32 promptHash;        // 正向 prompt 哈希
    bytes32 negativePromptHash; // 负向 prompt 哈希
    uint64 seed;               // 随机种子（关键：可重放验证）
    uint16 width;              // 图像宽度
    uint16 height;             // 图像高度
    uint8 steps;               // 推理步数
    uint16 cfgScale;           // CFG Scale (x100)
    bytes32 samplerHash;       // 采样器类型哈希

    // === 输出验证 ===
    bytes32 outputImageHash;   // 生成图像的 SHA256
    bytes32 latentHash;        // 中间 latent 哈希（可选，用于 ZK 验证）

    // === 防伪造 ===
    bytes32 requestSignature;  // Payer 对请求参数的签名
    bytes providerSignature;   // Provider 对完整 Receipt 的签名
}
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  方案 1：Seed 绑定验证（TEE 模式）                                │
│  ──────────────────────────────                                  │
│  1. Payer 生成随机 seed 并签名提交                                │
│  2. Provider 在 TEE 内使用该 seed 生成图像                        │
│  3. TEE 输出 (seed, outputImageHash) 的签名证明                   │
│  4. 仲裁时验证：TEE 签名有效 → 图像确实由指定 seed 生成            │
│                                                                  │
│  方案 2：ZK 可重放验证（ZK 模式）                                  │
│  ──────────────────────────────                                   │
│  1. Provider 在链下生成图像                                       │
│  2. Provider 生成 ZK 证明：                                       │
│     prove(prompt, seed, config → outputImageHash)                │
│  3. 链上验证 ZK 证明                                              │
│  4. 证明有效 → 确认输出由指定输入生成                              │
│                                                                  │
│  方案 3：时间戳 + 哈希链（轻量模式）                               │
│  ────────────────────────────────                                │
│  1. Payer 提交 (promptHash, seed) 并记录区块时间戳                │
│  2. Provider 在 N 分钟内提交 outputImageHash                      │
│  3. 若 Provider 提前知道 seed，无法预生成匹配图像                  │
│  4. 时间窗口内提交 → 高概率为真实生成                              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
#### 模板 3：API 调用计费（API Metering）

**场景**：SaaS API 按调用次数/token 数计费
...
struct APIUsageReceipt {
    ...
    // === API 计费专用字段 ===
    bytes32 endpointHash;      // API 端点哈希
    uint64 callCount;          // 调用次数
    uint64 totalUnits;         // 总消耗单位（token/字节/秒）
    bytes32 firstCallHash;     // 首次调用哈希
    bytes32 lastCallHash;      // 末次调用哈希
    bytes32 merkleRoot;        // 所有调用记录的 Merkle Root
...
**与 UsageReceipt 的映射建议**：

- `UsageReceipt.requestId/receiptId/amount` 对应 `APIUsageReceipt` 同名字段
- `UsageReceipt.responseHash` 建议取 `APIUsageReceipt.merkleRoot`
- `UsageReceipt.payloadHash` 建议对 `endpointHash/callCount/totalUnits/firstCallHash/lastCallHash/timestamp/payer/provider/amount` 做哈希
- `UsageReceipt.startTime/endTime` 可映射为首末调用时间窗口
- `UsageReceipt.aux` 预留模块自定义（如 endpointHash 或版本号）
...
每个 Call = hash(timestamp, endpoint, requestHash, responseHash, units)

争议时：
- Payer 可质疑任意 Call
- Provider 需提供 Merkle Proof 证明该 Call 确实发生
- 若无法提供 → 该 Call 不计费
```

---

#### 模板 4：电商物流交付（E-commerce Delivery）
...
struct DeliveryReceipt {
    ...
    // === 物流专用字段 ===
    bytes32 trackingNumber;    // 物流单号哈希
    bytes32 carrierHash;       // 物流公司标识
    bytes32 destinationHash;   // 目的地地址哈希
    uint64 shippedTime;        // 发货时间
    uint64 deliveredTime;      // 签收时间

    // === Oracle 数据 ===
    address[] oracleAddresses; // 确认交付的 Oracle
    bytes[] oracleSignatures;  // Oracle 签名
    bytes32 deliveryProofHash; // 签收凭证哈希（如签名照片）

    // === 可选：商品验证 ===
    bytes32 itemHash;          // 商品信息哈希
    bytes32 conditionHash;     // 商品状态哈希（完好/损坏）
}
```
...
| 争议类型 | 判定逻辑 | 仲裁模式 |
|----------|----------|----------|
| 未发货 | `shippedTime == 0` → Buyer 胜诉 | 自动 |
| 未签收 | `deliveredTime == 0` 且超时 → Buyer 胜诉 | Oracle |
| 货物损坏 | 查看 `deliveryProofHash` 照片 → Committee 判定 | Committee |
| 货不对版 | 比对 `itemHash` 与实际 → Committee 判定 | Committee |
...
struct EvidenceSchema {
    bytes32 schemaId;          // 唯一标识
    bytes32 moduleId;          // 关联的服务模块
    string name;               // 可读名称（如 "AI Image Generation v1"）
    bytes32 structHash;        // Receipt 结构的 EIP-712 类型哈希
    uint8 arbModeRequired;     // 推荐仲裁模式
    bytes32[] requiredFields;  // 必填字段列表
    bytes32[] optionalFields;  // 可选字段列表
    string specUri;            // 详细规范文档 URI
}
```

**模块开发者**需提交：
1. Receipt 结构定义（Solidity struct）
2. 字段验证逻辑（链上或链下脚本）
3. 仲裁判定规则文档
4. 示例 Receipt 和测试用例
...
| 等级 | 描述 | 证据要求 | 质押倍数 |
|------|------|----------|----------|
| **L1: 确定性可验证** | 给定输入，输出完全确定 | ZK proof / TEE attestation | 1.5x |
| **L2: 概率可验证** | 可重放但结果有随机性 | Seed + TEE / 多次重放比对 | 2x |
| **L3: Oracle 可验证** | 依赖外部数据源 | ≥2/3 Oracle 签名 | 2x |
| **L4: 主观可验证** | 需人工判断 | Committee 投票 | 3x |

**Provider 必须根据服务类型选择对应等级**，并满足相应质押要求。
...
| 参与方 | 操作 | Gas | 成本 |
|--------|------|-----|------|
| **Payer** | 发起争议 | 180,000 | ~$0.003 |
| **Payer** | 提交保证金 | （已包含） | - |
| **Provider** | 提交保证金 | 65,000 | ~$0.001 |
| **Payer** | 提交证据 | 95,000 | ~$0.002 |
...
| **Challenge Window** | 挑战期，结算前允许发起一次争议的时间窗口 |
| **Bond** | 保证金，争议双方需抵押的资金 |
| **Slash** | 罚没，违规方损失的质押 |
...
- v0.4: 扩展 Oracle 仲裁模式、添加 ProviderRegistry、完善接口定义、添加 Gas 成本分析和 Merkle 批量结算
